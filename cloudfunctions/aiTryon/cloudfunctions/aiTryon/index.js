...
// ---- V2 Try-On Engine（Phase 4.2 / 4.2.1，feature flag 控制，默认关闭以保留回滚能力）----
// 注意：aiTryon 不自行构造业务 Prompt。Prompt 由 Try-On Engine 内部 promptBuilder 负责。
...
function pickPersonSource(personAsset) {
  if (!personAsset || typeof personAsset !== "object") return { url: null, type: null, sourceKey: null };
  ...
  return { url: null, type: null, sourceKey: null };
}

/* ============================================================
   Body Profile 来源（Phase 4.2.1 修正）
   优先级：
     1. avatar_views.profile_snapshot 中已固化的真实身体字段（权威来源）
     2. Person Asset 的 bodyProfile / body_profile（仅补充快照缺失字段）
     3. null（绝不伪造）

   映射规则（下划线业务字段 → 驼峰 Try-On Context 字段）：
     height_cm       → heightCm
     weight_kg       → weightKg
   ...
   只映射实际存在且为合法 number 的字段；缺失保持 null，禁止 170cm/60kg 默认。
   ============================================================ */
const SNAPSHOT_TO_CONTEXT = [
  ["height_cm", "heightCm"],
  ["weight_kg", "weightKg"],
  ["shoulder_cm", "shoulderCm"],
  ["bust_cm", "bustCm"],
...
function mapSnapshotField(out, snapshot, snake, camel) {
  const v = snapshot[snake];
  if (typeof v === "number" && isFinite(v)) out[camel] = v;
}

/* 从 avatar_views.profile_snapshot 映射 bodyProfile（权威，含 gender） */
function mapProfileSnapshot(snapshot) {
  if (!snapshot || typeof snapshot !== "object") return {};
  const out = {};
  if (typeof snapshot.gender === "string" && snapshot.gender.length > 0) out.gender = snapshot.gender;
  for (const [snake, camel] of SNAPSHOT_TO_CONTEXT) mapSnapshotField(out, snapshot, snake, camel);
  return out;
}

/* 从 Person Asset 的 bodyProfile 补充（不覆盖快照已存在的字段） */
function mapPersonAssetBody(asset) {
  if (!asset || typeof asset !== "object") return {};
  const bp = asset.bodyProfile || asset.body_profile;
  if (!bp || typeof bp !== "object") return {};
  const out = {};
  if (typeof bp.gender === "string" && bp.gender.length > 0) out.gender = bp.gender;
  const camelOnly = ["heightCm", "weightKg", "shoulderCm", "bustCm", "waistCm", "hipCm", "legLengthCm", "armLengthCm", "neckLengthCm"];
  for (const camel of camelOnly) {
    if (typeof bp[camel] === "number" && isFinite(bp[camel])) out[camel] = bp[camel];
  }
  return out;
}

/* 合并 bodyProfile：快照优先，Person Asset 仅补充缺失字段；全空返回 null */
function buildBodyProfile(profileSnapshot, personAsset) {
  const fromSnapshot = mapProfileSnapshot(profileSnapshot);
  const fromAsset = mapPersonAssetBody(personAsset);
  const merged = Object.assign({}, fromAsset, fromSnapshot); // 快照覆盖 asset
  return Object.keys(merged).length ? merged : null;
}

/* ============================================================
   参考图转换：cloud:// 批量换临时链接。
   Fail Closed：任何 cloud:// 无法转换时抛 PROVIDER_ERROR，禁止静默丢弃。 */
...
/* 订阅消息通知（需配置环境变量 SUBSCRIBE_TMPL_ID） */
function fmtTime(ts) {
...
/* ============================================================
   解析当前请求对应的 Person Asset（Phase 4.2.1 修正）

   正确关联链路：
     avatarViewId
       → getOwnedDoc("avatar_views", avatarViewId, openid)   # ownership 已在上层校验
       → avatar_views.avatar_profile_id
       → person-asset.findByAvatarProfileId(avatarProfileId, openid)
           * 必须同时满足 user_id === openid（禁止跨用户）
           * 一个 profile 对应多个 asset 时，取 updated_at 最新且具可用人物照片者
           * 无匹配 → 返回 null（绝不使用"当前用户最新 Person Asset"顶替）

   返回 { asset, source }。asset 为 null 时表示需要走 legacy composite 兼容或 PERSON_ASSET_REQUIRED。
   ============================================================ */
async function resolvePersonAsset(openid, avatarViewId, avatarView) {
  // 1) 必须提供 avatarViewId，且对应的 avatar_views 已通过 ownership 校验
  if (!avatarView || typeof avatarView !== "object") {
    return { asset: null, source: null };
  }
  const avatarProfileId = avatarView.avatar_profile_id || avatarView.avatarProfileId || null;
  if (!avatarProfileId) {
    // 该 avatar_view 没有关联到任何 profile，无法精确匹配 Person Asset
    return { asset: null, source: null };
  }

  // 2) 通过 person-asset 服务按 avatar_profile_id + openid 精确查询（含 ownership）
  try {
    ...
    if (typeof service.findByAvatarProfileId === "function") {
      ...
      if (asset) {
        // 双重保险：最终 ownership 校验（防御性，服务层已校验）
        if (asset.user_id && asset.user_id !== openid) {
          console.log("resolvePersonAsset ownership mismatch", "assetUid=" + asset.user_id, "openid=" + openid);
          return { asset: null, source: null };
        }
        return { asset, source: "person_asset" };
      }
    } else if (typeof service.getPersonAsset === "function") {
      // 服务层未暴露精确查询时的兼容降级：仅按 id 精确匹配，绝不回退到"最新 asset"
      // （不调用 getCurrentPersonAsset，避免偷用其它 profile 的 asset）
      console.log("resolvePersonAsset: findByAvatarProfileId not available, precise-only mode");
      return { asset: null, source: null };
    }
  } catch (e) {
...
/* ============================================================
   ...
   构造标准 Try-On Context → tryonEngine.generate()
   注意：aiTryon 不构造 Prompt，由 Engine 内部负责。
   ============================================================ */
async function generateViaEngine({ personAsset, bodyProfile, garments, garmentName, strategy, openid, avatarViewId }) {
  ...
  // person_asset_id / version 用于 cache key 隔离
  ...
  console.log("aiTryon engine generate",
    "personSource=" + personSourceType,
    "garmentCount=" + engineGarments.length,
    "strategy=" + (strategy || "BALANCED"),
    "bodyProfile=" + (bodyProfile ? Object.keys(bodyProfile).join(",") : "null"));

  // aiTryon 不构造业务 Prompt —— Prompt 由 Engine 基于 context 自行构建
...
async function submit(event, openid) {
  ...
  // 人物三视图必须属于当前用户（ownership 校验）
  const av = await getOwnedDoc(db, "avatar_views", avId, openid);
  // avatar_views.profile_snapshot：Avatar Generation 已固化的真实身体档案（bodyProfile 权威来源）
  const profileSnapshot = av.profile_snapshot || null;
  ...
  const aigc = getAigc();
  const videoPrompt = buildTryonVideoPrompt(profileSnapshot, garmentName);

  // ---- 解析 Person Asset（按 avatarViewId → avatar_profile_id 精确关联，禁止偷用最新 asset）----
  const { asset: personAsset, source: personAssetSource } = await resolvePersonAsset(openid, avId, av);

  // ---- Body Profile：快照优先 → Person Asset 补充 → null（绝不伪造）----
  const bodyProfile = buildBodyProfile(profileSnapshot, personAsset);

  // cache key：必须含 personAssetId/version + avatarViewId，避免 composite 与 originalPhoto 共用缓存
  const personAssetId = personAsset && (personAsset._id || personAsset.assetId) ? String(personAsset._id || personAsset.assetId) : null;
  const personAssetVersion = personAsset && (personAsset.updated_at || personAsset.updatedAt) ? String(personAsset.updated_at || personAsset.updatedAt) : (personAssetId ? "v1" : "legacy");
  ...
  if (mode === "video") {
    ...
    if (lastErr) {
      ...
      await refundQuota(db, openid, dateStr());
  ...
  // consumeQuota 尚未执行，此处拒绝不扣 quota。
  const { url: personUrl, type: personSourceType, sourceKey: personSourceKey } = pickPersonSource(personAsset);
  if (isEngineEnabled()) {
    if (!personUrl) {
      // Engine 必须由真实 Person Asset 提供人物图，禁止 composite 冒充。
      // 若只有 composite（无 originalPhoto/frontPhoto/anchorImage）→ PERSON_ASSET_REQUIRED
      console.log("aiTryon engine preflight no usable person source", "openid=" + openid, "hasComposite=" + (!!((av.views || {}).composite)), "costMs=" + (Date.now() - t0));
      return { ok: false, error: "PERSON_ASSET_REQUIRED", message: "请先上传真实人物照片以建立人物资产" };
    }
  }

  // ---- Reference Preflight（必须在 consumeQuota 之前完成，保证 preflight 失败不扣 quota）----
  const preflightRefs = [];
  if (isEngineEnabled()) {
  ...
  // 所有 preflight（person asset / reference）均在 consumeQuota 之前完成，故天然不扣 quota。
  const date = dateStr();
  let quota = null;
  try {
  ...
  const imagePrompt = buildTryonImagePrompt(profileSnapshot, garments.map((g) => g.name), refImages.length);
  const addRes = await db.collection("tryon_tasks").add({ data: task });
  const taskId = addRes._id;
  assertTransition("queued", "processing");
  await db.collection("tryon_tasks").doc(taskId).update({ data: { status: "processing", updated_at: Date.now() } });
