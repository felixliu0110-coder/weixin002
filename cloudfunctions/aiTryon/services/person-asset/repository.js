...
class PersonAssetRepository {
  ...
  /**
   * 确保一个 avatar profile 至少有一份与当前用户精确绑定的人物资产。
   * 只在没有现存 Person Asset 时创建；已有资产只补齐照片引用，不创建重复资产。
   */
  async ensureForAvatarProfile(avatarProfileId, openid, photos = {}) {
    if (!avatarProfileId || !openid) return null;
    const existing = await this.findByAvatarProfileId(avatarProfileId, openid);
    const originalPhoto = photos.originalPhoto || photos.original_photo || '';
    const frontPhoto = photos.frontPhoto || photos.front_photo || '';
    if (existing) {
      const patch = {};
      if (!existing.original_photo && originalPhoto) patch.original_photo = originalPhoto;
      if (!existing.front_photo && frontPhoto) patch.front_photo = frontPhoto;
      if (Object.keys(patch).length === 0) return existing;
      return this.update(existing._id, openid, { ...patch, status: ASSET_STATUS.READY });
    }
    if (!originalPhoto && !frontPhoto) return null;
    const now = Date.now();
    const doc = {
      user_id: openid,
      openid,
